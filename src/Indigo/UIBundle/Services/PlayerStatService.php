...
class PlayerStatService implements LoggerAwareInterface
{
    ...
    public function getPlayerContestStat($contestId)
    {
        $teamsStats = new \ArrayObject();
        ...
        foreach ($games as $game) {
            ...
            if ($teamId && !isset($teamsStat[$teamId])) {

                @$teamsStats[$teamId] = new PlayerTeamStatModel();
                $teamsStats[$teamId]->setTeamRating($teamEntity->getTeamRatings($contestId));
            }
            /** @var PlayerTeamStatModel $teamStat */
            $teamStat = $teamsStats[$teamId];
        ...
        if ($teamsStats) {
            ...
            try {
                ...
                $teamId = $singleTeam->getId();

                @$teamsStats[$teamId] = new PlayerTeamStatModel();
                $teamsStats[$teamId]->setTeamRating($singleTeam->getTeamRatings($contestId));
